...
checkRegistered(S, [{S1, _, Al, _} | T]) ->
    if S == S1 -> registered;
       S /= S1 -> Member = lists:member(S, Al),
...
loop(Initial) ->
    receive
        {From, {new, {S1, E1}}} ->
            ...
            if  Found /= not_found ->
                    From ! {self(), {error, "The shortcode "++ S1 ++" already exists"}},
                    loop(Initial);
                Found == not_found ->
    ...
    {From, {alias, {S1, S2}}} ->
            ...
            if  Found == not_found ->
                    ShortCode = getShortcodeForAlias(S1, Initial),
                    if ShortCode == no_shortcode ->
                            From ! {self(), {error, "The shortcode "++ S1 ++" does not exists"}},
                            loop(Initial);
                       ShortCode /= no_shortcode ->
                            ...
                            if  FoundAlias =:= false ->
                                    ...
                                    loop(NewInitial);
                                FoundAlias =:= true ->
                                    From ! {self(), {error, "The alias "++ S2 ++" already exists"}},
                                    loop(Initial)
                            end
                    end;
                Found /= not_found ->
                    ...
                    if Registered == registered ->
                       ...
                       Registered == not_registered ->     
                            ...
                            FoundAlias = lists:member(S2, Al),
                            if  FoundAlias =:= true ->
                                    From ! {self(), {error, "The alias "++ S2 ++" already exist"}},
                                    loop(Initial);
                                FoundAlias =:= false ->
